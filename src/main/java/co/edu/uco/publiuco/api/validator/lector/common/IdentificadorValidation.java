...
public class IdentificadorValidation implements Validation<UUID> {
	...
	public Result execute(UUID data) {
		...
		if(UtilObject.isNull(data)) {
			result.addMessage("No es posible continuar con el identificador del lector vacío");
			
		}else if(UtilUUID.isDefault(data)) {
			result.addMessage("No es posible tener el identificador por defecto del lector");
		}
		return result;
	}
