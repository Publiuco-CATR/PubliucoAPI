...
public class SegundoNombreValidation implements Validation<String>{
	...
	public Result execute(String data) {
		...
		if(UtilText.isEmpty(data)) {
			result.addMessage("No es posible continuar con el segundo nombre vacío");
			
		}else {
			
			if(!UtilText.getUtilText().textHasLenghtAllowed(data, MINIMUN_LENGHT, MAXIMUN_LENGHT)) { 
				result.addMessage("El segundo nombre no puede ser menor a 1 o mayor a 30 caracteres");
			}
			if(!UtilText.getUtilText().textHasOnlyLetters(data)) { 
				result.addMessage("El segundo nombre solo puede tener letras y espacios");
			}
