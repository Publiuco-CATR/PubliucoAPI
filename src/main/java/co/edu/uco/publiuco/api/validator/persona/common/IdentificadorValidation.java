...
public class IdentificadorValidation implements Validation<UUID> {
	...
	public Result execute(UUID data) {
		...
		if(UtilObject.isNull(data)) {
			result.addMessage("No es posible continuar con el identificador de la persona vacío");
			
		}else if(UtilUUID.isDefault(data)) {
			result.addMessage("No es posible tener el identificador por defecto de la persona");
		}
		return result;
	}
